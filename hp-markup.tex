...
% Letters
\newcommand{\letterAddress}[1]{\pagebreak[1]\par\fontspec[ExternalLocation]{Graphe_Alpha_alt.ttf}\scriptsize#1\nopagebreak[4]\par\noindent}
\newcommand{\letterClosing}[2][\vskip 1\baselineskip]{\nopagebreak[4]\fontspec[ExternalLocation]{Graphe_Alpha_alt.ttf}\scriptsize#1\par\nopagebreak[5]\noindent\hfil\hfil\hfil#2\hfil}
...
\newenvironment{theGame}{\pagebreak[1]\par\begin{center}\begin{minipage}{.85\linewidth}\bgroup\ttfamily}
  {\egroup\end{minipage}\end{center}}
\newcommand{\gameResult}[2]{\hbox{\textsc{#1}~:}&\hbox{~\textsc{#2}}\\}
\newcommand{\gameResultnnl}[2]{\hbox{\textsc{#1}~:}&\hbox{~\textsc{#2}}}
\newenvironment{theGameResults}{\begin{theGame}}
  {\end{theGame}}
\let\gameInstruction=\textsc
  % \start@align\@ne\st@rredtrue\m@ne}
  % {\endalign\end{theGame}}
...
\newcommand{\partchapter}[3][\relax]{%
	\chapter[\texorpdfstring{#2, \abbrev{partie #3}}{#2, partie #3}]%
		[#2 #3]{#2, Partie~#3#1}}
...
\newcommand{\aftermaths}{\latersection{Épilogue}}
\makeatother
